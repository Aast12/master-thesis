...
\Fn{\FBatchGetExclusiveMasks{$pred$, $threshold$}}{
    ...
    $groups_count \leftarrow \text{len}(\text{MASK\_GROUPS})$\;
    ...
    \KwRet{$ordered\_groups, exclusive\_masks$}\;
}
...
\Fn{\FGetGroupMask{$pred$, $group$, $threshold$}}{
    ...
    $thresholded\_masks \leftarrow \mathbb{1}(normalized\_masks > threshold)$\;\\
    $joined\_masks \leftarrow \text{sum}(thresholded\_masks, axis=1)$\;\\
    \KwRet{$\text{clip}(thresholded\_masks, 0.0, 1.0)$}\;
}

\caption{Algorithms for creating exclusive non-overlapping segmentation masks}

...
% \sweExpl{Vetenskaplig och ingenjörsmässig frågeställning}
some text
...
\begin{enumerate}
\item Subgoal 1 \sweExpl{för att tillfredsstä2lla problemägaren – industrin?}
...
For the development of the framework, the input data consists of images taken from the CheXpert dataset \cite{chexpert} and NIH Chest X-ray dataset \cite{nih}. For the implementation of segmentation-based regularizations, ChestXDet \cite{chestxdet}, an existing segmentation model was used to obtain segmentation labels. Concretely, the segmentation model identifies 13 categories: Atelectasis, Calcification, Cardiomegaly, Consolidation, Diffuse Nodule, Effusion, Emphysema, Fibrosis, Fracture, Mass, Nodule, Pleural Thickening, Pneumothorax. These labels represent smaller structures in the chest anatomy, and can be grouped according to similar expected attenuation coefficients. For the model, these groups are considered.

\begin{table}[]
    \centering
    \begin{tabular}{c|c}
        Group name & Grouped labels \\
        Bone &  "Left Clavicle", "Right Clavicle", "Left Scapula", "Right Scapula", "Spine" \\
        Lung &     "Left Lung", "Right Lung", "Left Hilus Pulmonis", "Right Hilus Pulmonis" \\
        Soft tissue &  "Heart", "Aorta", "Mediastinum", "Facies Diaphragmatica", "Weasand"
    \end{tabular}
    \caption{Segmentation category groups}
    \label{tab:segmentation_groups}
\end{table}

% TODO: Change this ref
\ref{sec:implementationDetails} 

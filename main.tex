...
iat KTH, one paired with a processed image produced by a clinical-grade detector. \autoref{fig:chestPhantomImages} shows the obtained sample
...
\begin{description}
  ...
  \begin{figure}[H]
   ...
   chest. These are limited to structures we can identify via an existing segmentation model, as defined in \autoref{sec:segmentation}.
    ...
    and processed image. Processed images tend to have a flattened histogram as consequence of
    contrast enhacement operations. In contrast, the transmission map histogram has higher
...
\begin{itemize}
    ...
    \item If we recover an appropriate image processing model that can produce images perceptually
...
\begin{equation}
    ...
    \min_{x_i, \theta_i \in \Omega_\theta}  \underbrace{\mathcal{D}(y_i, F(x_i, \theta_i)) + \lambda_{\text{smooth}} R_{\text{smooth}}(x_i) + \lambda_{\text{anatomy}} R_{\text{anatomy}}(x_i)}_{=: \ell(\cdot)}.
\end{equation}

$\mathcal{D}$ is a data fidelity function measuring the discrepancy between the processed versions of
our estimated transmission maps and the observed processed images, becoming zero only when \autoref{eq:dataFidelity}
...
One expectation of the images $x_i$ is for them to not have discontinuities or noise. An approach to promote that is
to introduce a smoothness regularization term $R_{\text{smooth}}$, weighted by $\lambda_{\text{smooth}}$.
In \autoref{sec:transmissionMaps} I also discussed broad constraints the images should meet, regarding
the range of the transmission values, particularly in regions where the traversing tissues have close absorption
coefficients. This anatomical value ranges can't be strictly enforced due to the diversity of body compositions,
...
Our transformation $F$ can be  further constrained by restricting the parameters $\theta_i$ to take values
...
digital radiography processing pipeline. The goal of building such function is so it can produce
...
\item[Negative logarithm] $F_{1}(x) = -\log(x + \epsilon)$, where $\epsilon$ is a small constant to avoid numerical instability on values close to zero.
    ...
    However, processed images operate on the densities term $\mu x$, explaining the color-negative relationship between
    transmission maps and diagnostic images.
...
Regarding the frequency band represented by $\sigma$, the implementation considers a fixed value, or a
range that will capture higher-frequencies, following the recommendation of targeting a 20-30mm kernel
size \cite[p.~153]{Prokop2003}.
...
Despite this caveat, it is still a choice to compute data fidelity due to its simplicity and computational efficiency.
Other metrics are used to evaluate and compare the performance of the model, as having accuracy in different frequency bands of our
image is critical to claim the model yields a valid solution.
...
\begin{description}
    ...
    is a regularization for noise removal that preserves sharp edges, defined as
      \begin{equation}
        R_{TV}^{aniso}(x) = \sum_{i,j} |x_{i+1,j} - x_{i,j}| + |x_{i,j+1} - x_{i,j}|.
    \end{equation}

    For transmission maps, TV regularization is particularly suitable as it promotes recovering images with sharp edges,
    ...
    While Tikhonov regularization avoids the staircasing artifacts associated with TV, it tends to over-smooth edges, potentially obscuring
    important anatomical boundaries. The choice between TV and Tikhonov regularization thus represents a trade-off between edge preservation
    and smoothness in homogeneous regions. Despite this caveat, it is expected to lose sharp edges from the assumption of operations
    in the high-frequency space, which can make it an appropriate choice to our case.

    \item[Anatomical Regularization] Leveraging the segmentation masks described in Section \ref{sec:segmentation}, we incorporate anatomically-informed
    constraints that encode the expected transmission characteristics of different tissue types.
    For each anatomical group $t \in \{\text{bone}, \text{lung}, \text{soft}\}$, we define region-specific penalties:

    \begin{equation}
        R_{anatomy}(x_i) = \sum_{t} \sum_{(j,k) \in S_i^t} \rho_t(x_i(j,k))
    \end{equation}

    where $S_i^t$ denotes the set of pixels belonging to anatomical group $t$, and $\rho_t$ is a penalty function defined as:
    ...
    The bounds $[v_{\min}^t, v_{\max}^t]$ are derived from the analysis of real transmission maps as described in Section \ref{sec:transmissionMaps}. This soft constraint formulation allows for natural variations in tissue density while penalizing physically implausible transmission values. The normalization by $|S_i^t|$ ensures that the penalty contribution is independent of the region size, preventing bias toward larger anatomical structures.

    \item[Adaptive Weighting Strategy] The segmentation confidence values from the ChestXDet model can be incorporated to create spatially-varying regularization weights:

    \begin{equation}
        \lambda_{anatomy}(j,k) = \lambda_{base} \cdot \max_{t} c_i^t(j,k)
    \end{equation}

    where $c_i^t(j,k) \in [0,1]$ represents the confidence of pixel $(j,k)$ belonging to anatomical group $t$. This adaptive approach applies stronger regularization in regions with high segmentation confidence while allowing more flexibility in uncertain regions, such as tissue boundaries.

    \item[Frequency-Selective Regularization] Given that transmission maps exhibit suppressed high-frequency content compared to processed images (Section \ref{sec:transmissionMaps}), we consider frequency-selective regularization:

    \begin{equation}
        R_{freq}(x) = \sum_{\omega \in \Omega_{high}} |\hat{x}(\omega)|^2
    \end{equation}

    where $\hat{x}$ denotes the Fourier transform of $x$ and $\Omega_{high}$ represents the high-frequency domain. This penalty directly suppresses high-frequency components that are unlikely to be present in real transmission maps but may arise from noise or optimization artifacts.
\end{description}

The complete regularization functional combines these terms with appropriate weighting:

\begin{equation}
    R(x_i, s_i) = \lambda_{smooth} R_{smooth}(x_i) + \lambda_{anatomy} R_{anatomy}(x_i, s_i) + \lambda_{freq} R_{freq}(x_i)
\end{equation}

where $R_{smooth} \in \{R_{TV}, R_{Tikh}\}$ is selected based on the desired trade-off between edge preservation and smoothness. The regularization weights $\{\lambda_{smooth}, \lambda_{anatomy}, \lambda_{freq}\}$ control the relative importance of each term and are determined through hyperparameter optimization as described in Chapter 4.

The interplay between these regularization terms addresses different aspects of the ill-posedness: smoothness regularization stabilizes the inversion by restricting the solution space, anatomical regularization incorporates domain-specific knowledge to ensure physical plausibility, and frequency-selective regularization suppresses artifacts that do not conform to the expected spectral characteristics of transmission maps. This multi-faceted approach enables the recovery of transmission maps that are not only consistent with the observed processed images but also adhere to the physical constraints governing X-ray transmission through human tissue.}

\begin{description}
    \item[Smoothing regularization] Two alternatives of smoothing regularization are considered:

   \acrfull{TV}, defined as

   \begin{equation}
    TV(x) =  \sum_{i, j} \left( \lvert x_{i + 1, j} - x_{i, j} \rvert + \sum_{i, j} \lvert x_{i, j + 1} - x_{i, j} \rvert \right)
    \end{equation}

    prompiecewise smoothness in $x_i$, thus discouraging discontinuities
...
and processing operator are ruled by hyperparameters fixed for the entire iterative process.
...
It was mentioned previously that \arcshort{MSE} does not necessarily favor solutions that are perceptually close
to the original images. There are multiple alternative loss functions designed specifically for Image Quality Assessment
...
\begin{equation}
\begin{aligned}
    0.1 \leq & \text{low_enhance_factor} &<= 0.8 \\
    0.3 \leq & \text{high_enhance_factor} &= 1.0 \\
    0.1 \leq & \text{window_center} &<= 0.8 \\
    0.1 \leq & \text{window_width} &<= 1.0 \\
\end{aligned}
\end{equation}

These values are set empirically, based on the assumption that high-frequencies are always amplified to some degree,
while not a necessity for low-frequencies. The windowing params are set arbitrarily to allow centering in the first
...
\section{Trivial transformation}

One way of approximating the transmission map of a transmission map trivially is to compute a negative transformation of the image,
and squeezing the image range to effectively give an image that is close to the expected anatomical bounds.

...
\section{Processing Operator Model}
...
some edge enhancement, the lower bound can be increased to a non-zero value to assume some edge enhancement is
being applied in the processed image. Regarding the frequency band represented by
...
Despite this caveat, it is still the choice to represent data fidelity due to its simplicity and computational efficiency.
However, other metrics are used to evaluate and compare the performance of the model, as having accuracy in
different frequency bands of our image is critical to claim the model yields a valid solution.

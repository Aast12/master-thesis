...
\todo{add image acquisition diagram}
...
and the material encounterd\cite[p.~11]{cullityElementsXrayDiffraction2014}. The intensity
...
\item $I_0(x,y)$ denote the intensity measured at detector position $(x,y)$ in the absence
    of any object (the reference or flat-field measurement)
...
incident radiation that transmits through the patient at each position $(x, y)$.
...
The x-ray image is created by the detection of the transmitted x-rays of an incident, uniform x-ray beam
as they pass through an object. Note that ideal x-ray image acquisition and the described
model assume a point x-ray source, a straight line trajectory from the source through the object,
and complete detection of the x-ray beam that strikes the detector. These are not realistic assumptions, however,
clinical detectors apply methods to counteract nonideal conditions \cite[p.~9]{Seibert3}.
...
different coefficients. More precisely, \autoref{fig:attenuation_coefficients} shows the attenuation coefficients
of common body tissue types. The table is presented with a dimensionless measure called a
Hounsfield unit, which is a measure relative to the attenuation coefficient of water defined as \cite[p.~54]{epstein2008}

...
trained on high-quality datasets like CheXpert may not perform adequately on images captured from these systems. To address the later, new models would be needed
that are specifically trained on the unique characteristics of the proposed system.

\section{Problem Statement}
\subsection{Original Problem Definition}
The core problem is to build an open-source X-ray detector that is cost-effective yet diagnostically useful.  However, all film and digitalX-ray detectors and their subsequent processing pipelines operate on the same transmission maps,
which represent the incident radiation (still dependent on dose and exposure time), which is later transformed by the detector system, and later processed to obtain
...
and the development of diagnostic models for the specific profile of images captured from these systems. This describes the primary goal of this project: to develop a framework that
inverts the image processing transformations from high-quality chest x-ray images to obtain a new dataset of their underlying transmission maps.

Morevever, equipment manufacturers (e.g., Siemens, Philips, GE) apply proprietary post-processing algorithms that include non-linear
...
and images, represented in large datasets like CheXpert.
...
\begin{itemize}
    ...
    \item \textbf{Diversity of processing approaches}:
    ...
    \item \textbf{Scaling requirements}: We need to process many images to build a representative dataset.
\end{itemize}

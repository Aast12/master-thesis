...
digital image processing, and the an overview of the optimization methods applied to solve this problem. Chapter 3
...
% A fundamental tool for problems in science and engineering, and particularly in medical imaging, is
% nonlinear optimization. In medical imaging, and overall problems in image reconstruction, it is common
% to formulate the problem as an optimization problem, to find out of a space of images, one that
% maximizes or minimizes an objective and satisfies some defined criteria.

% Optimization refers precisely to the minimization or maximization of a function subject
% to constraints on its variables. Consider a called \textit{objective function} $f$ as a scalar function with
% variables $x$, and a set of constraint scalar functions $c_i$ define equations and inequalities
% that $x$ must satisfy \cite[p.~2]{NumericalOptimization2006}. With this notation, an optimization problem
% can be formulated as:

% \begin{equation}
%     \begin{aligned}
%         \min_{x \in \Omega \quad & f(x) \\
%         \text{subject to} \quad & c_i(x) \leq 0, \quad i \in \mathcal{I} \\
%         & c_i(x) = 0, \quad i \in \mathcal{E}.
%     \end{aligned}
% \end{equation}

% Here, $\mathcal{I}$ and $\mathcal{E}$ are the set of indices corresponding to inequality and equality constraints, respectively.
% There exist many methods for the solution of these problems, although in most cases, and especially when the
% objective function is non-linear and non-convex, only seek for a local solution. Reaching a global solution
% is often difficult


% \section{Related Work}
% \subsection{Inverse Problems in Medical Imaging}
% While image recovery from processed versions has been studied in photography \cite{moran2020deeptone}, similar
% work in medical imaging is limited. Most related work focuses on denoising, deblurring, or super-resolution rather
% than recovering physically meaningful quantities like transmission maps \cite{jin2017deep}.
...
We had access to, precisely, two images taken from the same chest phantom at the Hard X-ray lab in KTH \todo{is this the right source},
...
\begin{description}
   ...
   defined bounds (if any). More formally, let $s_i^{t}(x, y)$ denote the binary mask for $x_i$ of the anatomical group $t$,
   and $v^{t}_{min}, v^{t}_{max}$ be the expected transmission bounds for said group. Then,

   \begin{equation}
   p_t(x_i, s^t_i) = \frac{\sum_{j, k}  s_i^t(j, k) \cdot \left[ \max(0, x_i(j, k) - v^{t}_{max})^2 + \max(0, v^{t}_{min} - x_i(j, k) )^2\right]}{\sum_{j, k}  s_i^t(j, k) }
   \end{equation}

...
For this thesis project, a family of methods \todo{is this the right term} are explored to recover transmission maps under the lack of prior knowledge about the ground truth, and the
...
\begin{description}
    ...
    \begin{equation}
        \min_{X, \{\theta_i\}_{i=1}^{N}} \frac{1}{N}\sum_{i=1}^{N} \ell(y_i, F(x_i, \theta_i)) + \lambda R(X),
    \end{equation}
    where we now have a set of parameters $\theta = \{\theta_1, \theta_2, \dots, \theta_N\} \text{ with }\theta_i \in \mathbb{R}^{d}$.
\end{description}


The method employed in this project involved an iterative process of defining a loss function and regularization, which can be applied to all described model variations,
and finding a framework that can yield results considered realistic.

\section{Data Collection and Preprocessing}
\label{sec:dataCollection}

\begin{description}
    \item[Transmission Maps] There is a limited amount of real samples taken

    \item[Diagnostic chest X-ray images] For the development of the framework, the input data consists of images taken from
    the CheXpert dataset \cite{chexpert} and NIH Chest X-ray dataset \cite{nih}.

    \item[Segmentation] For the implementation of segmentation-based regularizations,
    ChestXDet \cite{chestxdet}, an existing segmentation model was used to obtain segmentation
    labels. Concretely, the segmentation model identifies 13 categories: Atelectasis,
    Calcification, Cardiomegaly, Consolidation, Diffuse Nodule, Effusion, Emphysema,
    Fibrosis, Fracture, Mass, Nodule, Pleural Thickening, Pneumothorax. These labels represent
    smaller structures in the chest anatomy, and can be grouped according to similar expected
    attenuation coefficients. For the model, these groups are considered.
\end{description}

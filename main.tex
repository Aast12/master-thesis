...
\cite[p.~119]{carterDigitalRadiographyPACS2010}, and is also worth noting that iterations of such algorithms typically
...
approaches become infeasible. However, the patterns studied in \arcshort{DIP} algorithms hints to the possibility
of injecting prior knowledge into a learning process by means of the transformations the processed images undergo.
The assumption then is that, if there is a known image processsing model that can produce images perceptually similar to the
...
This can be formulated as an optimization problem: for each image $y_i$, it is looked for a pair of a transmission map $x_i$
...
coefficients. This anatomical value ranges can't be set as strict constraints due to the diversity of body compositions,
...
\begin{description}
    \item[Common operator parameters] If it is assumed that there exist a single set of parameters $\theta$
...
Other metrics are used to evaluate and compare the performance of the model, as having accuracy in different scales of the
...
\begin{description}
    ...
    is a regularization for noise removal that preserves sharp edges, annd can be defined as
      \begin{equation}
    ...
    The risk of Tikhonov regularization is a tendency to over-smooth edges, potentially obscuring important anatomical boundaries.
...
and squeezing (e.g. scaling to the range [0, 0.5]) the image range. This will effectively give an image that is close to the expected
...
be useful as a baseline for evaluating against ground truth, even though there is a single pair of such data. This transformation
also resulted to be a good initial value for the optimization, so unless otherwise specified, for the hyperparameter searches done
it is assumed that the images are initialized with this transformation.

An additional note on this transformation is that these kind of images can't be considered physically plausible, as they present
...
problems at once is possible to speed up computing, but the parameter updates are still independent across images. The second appoach
...
Even though the current project could find solutions that, accrding to our criteria
